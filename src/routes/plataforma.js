...
router.get("/:id", async (req, res) => {
  try {
    ...
    const r = await db.query("SELECT * FROM plataformas WHERE id=$1;", [id])
    return res.status(200).json(r.rows)
  } catch (error) {
    return res.status(400).json({ msg: error })
  }
...
router.put("/:id", async (req, res) => {
  try {
    const id = req.params.id
    const verificaPlataforma = await db.query("SELECT * FROM plataformas WHERE id=$1", [id])
    if (verificaPlataforma.rowCount == 0) {
      return res.status(400).json({ msg: "plataforma não encontrado" })
    }
    const { nome } = req.body || {};
    if (!nome) {
      return res.status(404).json({ msg: "nome não enviado, não editado!" })
    }
    const r = await db.query("UPDATE plataformas SET nome=$1 WHERE id=$2 RETURNING *", [nome,id])
    return res.status(200).json({
      msg: "plataforma editada!",
      plataforma: r.rows[0]
    })
  } catch (error) {
    throw new Error(error)
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (v.rowCount == 0) {
      return res.status(404).json({ msg: "plataforma nao encontrada, não deletado!" })
    }
